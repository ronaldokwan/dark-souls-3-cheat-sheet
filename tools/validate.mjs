...
const FILTER_CLASSES = new Set([
  'f_none', 'f_boss', 'f_miss', 'f_npc', 'f_estus', 'f_bone', 'f_tome', 'f_coal',
...
function validateItem(item, where, key) {
  ...
  if (seenIds.has(item.id)) err(where, `duplicate id "${item.id}" (also at ${seenIds.get(item.id)})`);
  ...
  if (item.cls !== undefined && item.cls !== '') {
    if (typeof item.cls !== 'string') return err(where + ' [' + item.id + ']', 'cls must be a string');
    item.cls.trim().split(/\s+/).forEach(function (c) {
...
data.tabs.forEach(function (tab, ti) {
  ...
  if (!Array.isArray(tab.sections)) { err(tw, 'missing "sections" array'); return; }
  ...
  tab.sections.forEach(function (s, si) {
    ...
    if (s.type === 'items') {
      ...
      else s.items.forEach(function (it) { validateItem(it, sw, tab.key); });
    } else if (s.type === 'groups') {
      if (!Array.isArray(s.groups)) err(sw, 'groups section missing groups array');
      else s.groups.forEach(function (g, gi) {
        ...
        else g.items.forEach(function (it) { validateItem(it, `${sw} groups[${gi}]`, tab.key); });
      });
    } else if (s.type === 'raw') {
